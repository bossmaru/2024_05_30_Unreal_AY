...
void Player::DIjkstra(Vector2 start)
{
	...
	vector<vector<Vector2>> parent = vector<vector<Vector2>>(MAXCOUNT_Y, vector<Vector2>(MAXCOUNT_X, Vector2(-1, -1)));
	vector<vector<int>>best = vector<vector<int>>(MAXCOUNT_Y, vector<int>(MAXCOUNT_X, INT_MAX));
...
void Player::Astar(Vector2 start, Vector2 end)
{
	...
	startV.pos = start;
	startV.g = 0;
	startV.h = start.ManhattanDistance(_maze->GetEndPos()) * 10;
	...
	while (true)
	{
		...
		for (int i = 0; i < 8; i++)
		{
			...
			int h = there.ManhattanDistance(_maze->GetEndPos()) * 10;
			int f = newCost + h;
...
void Player::DFS(Vector2 start)
{
	...
	for (int i = 0; i < 8; i++)
	{
		...
		if(Cango(there._y, there._x) == false)
			continue;
		if (_visited[there._y][there._x] == true)
			continue;
