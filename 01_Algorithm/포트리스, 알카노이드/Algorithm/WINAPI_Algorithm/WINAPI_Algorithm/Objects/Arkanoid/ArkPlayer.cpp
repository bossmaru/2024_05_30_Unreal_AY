...
void ArkPlayer::GetItem(vector<vector<shared_ptr<Brick>>> bricks)
{
	for (auto brick_X : bricks)
	{
		for (auto brick : brick_X)
		{
			...
			if (item)
			{
				if (item->GetItemType() == ItemType::LongBar && item->IsCollision(_bar))
				{
					_bar->ChangeSize(Vector2(Bar_Width * 1.15, Bar_Height));

				}
				else if (item->GetItemType() == ItemType::DoubleBall && item->IsCollision(_bar))
				{
					for (auto ball : _bonusBalls)
					{
						if (!ball->_isUsed)
				...
				else if (item->GetItemType() == ItemType::TripleBall && item->IsCollision(_bar))
				{
					for (int i = 0; i < 2; i++)
					{
						for (auto ball : _bonusBalls)
